...
调试沿用现有 Executor Workflow 限制：只支持四种标量 Schema；object、array、script Step、CLI 设备角色和其他无法投影的结构会在启动前失败。公共投影会静默忽略 Log/Config CollectionCall，但当前被调试 Step 只要包含这两类采集就禁止启动，并返回 `workflow_debug.unsupported_collection_type`；其他 Step 的 Log/Config 不影响纯 CLI Step 调试。案例、通用采集 fixture 和历史在禁用状态下仍可维护。
