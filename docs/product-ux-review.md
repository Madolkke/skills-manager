...
- **Linear Saved Views / Airtable Views:** 常用筛选、列显示和比较上下文不应该每次重建，视图保存的是查询意图和展示偏好而不是复制数据。SkillHub 适配为保存 `run_history` 筛选配置、matrix 展示偏好、`Impact` 列可见性和对照/候选 run 指针，run 列表、矩阵和比较结果仍实时读取同一份后端结果。
