...
- `workflows.document_schema_version` 标识持久化格式；当前只接受 schema v2，`WorkflowBundle` 内不保存格式版本。
...
### Parameter 与 StepInput

Parameter 声明参数槽位，包含 `id/key/name/description/dataType/required`。StepInput 将 Parameter 与可选 Binding 组合，不保存运行时值。
...
| `step_input` | `{ input_id }` | 当前 Step 输入 |
...
CollectionDefinition 包含稳定 `id + revision`、元信息、输入、输出、类型专属 spec 和可选 `forkedFrom`。当前唯一采集类型是 CLI。
...
CollectionCall 表示某个 Step 对 CollectionDefinition 精确版本的一次使用，保存可选调用 Key/Name、Definition 引用、设备角色、样本数量和参数绑定。调用名称为空时展示 Collection 名称；调用 Key 非空时作为输出字段命名空间，例如 `status.version`，为空时直接暴露输出字段。直接暴露的字段不得与当前步骤输入、Workflow 全局输入或其他直接暴露输出重名。
...
1. 新定义继承 Workflow 的产业、设备和适用版本，并复制当前步骤输入为 Collection 输入，自动建立步骤输入绑定。
...
- Step 是判断节点，可为 `expression` 或 `script`。跳转条件表达式支持 `global.<key>`、`step.<key>` 和 `output.<callKey>.<outputKey>` 变量补全；补全覆盖所有步骤已定义的采集输出，仅用于辅助输入，不承担运行时校验。
...
典型错误包括：无起始步骤、重复 ID、参数 Key/名称缺失、参数或采集 Key 重复、多行采集命令、无命名空间输出冲突、无效节点或 Collection 引用、缺少必填绑定、无效设备角色和样本数量小于 1。
