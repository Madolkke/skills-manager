...
本文档用于将已运行的 SkillHub 环境从 Alembic revision `0003_workflow_skill_generators` 升级到 `0004_workflow_json_schema_v4`。仍停留在 `0002_skill_identity_global_admin` 的环境必须连续执行 `0003` 与 `0004`，不得跳过 Generator evidence 回填。本次升级会重写当前 Workflow JSONB，并为仍在使用的 Collection revision 创建 v4 副本，因此必须按停写、备份、单次迁移、验证、恢复流量的顺序执行。

## 1. 适用范围与限制

- 目标数据库必须是 PostgreSQL。
- 推荐的迁移前 revision 是 `0003_workflow_skill_generators`。
- API、Worker 和 Web 不支持新旧版本混跑。
- `0004` 不支持 downgrade。回退必须同时恢复旧版应用和迁移前数据库备份。
...
```text
0003_workflow_skill_generators
```

如果数据库是 `0001_initial_schema` 或 `0002_skill_identity_global_admin`，必须先在生产副本上演练连续升级。`0002` 升级时会先执行 `0003_workflow_skill_generators`，回填并保留历史 Generator evidence，再执行不可逆的 `0004`。如果 revision 为空、未知或存在多 head，停止部署并先完成数据库基线审计。
...
预期结果是 `0004_workflow_json_schema_v4`。
...
`0004` 的 `downgrade()` 会直接拒绝执行，因此备份和恢复演练是本次部署的强制步骤。
