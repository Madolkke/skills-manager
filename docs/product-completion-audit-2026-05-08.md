...
状态：尚未达到“成熟产品完成”。当前已经是一个强的正式垂直切片：主工作区 Skill Launchpad、移动端 first-run 单主路径、中等桌面证据视图 compact inspector rail、URL state 第二阶段、高频写入表单字段基础件第二阶段、表单验证错误摘要、表单错误摘要统计、低频长文本字符计数、后端字段错误映射、基础格式校验第一阶段、导入 bundle 字段错误映射第一阶段、批量 case 行级错误第一阶段、服务端批量 case 字段错误契约、EvalRun results 精确字段校验、eval case 文本长度校验、批量 case 导入预览表、批量 case 预览移动端护栏、保存视图名称字段级校验、accepted verification note 字段级校验、promotion decision note 字段级校验、Variant 写入字段校验、Skill capabilities 权限感知、本地登录门禁第一阶段、本地 session 退出控制、身份引用字段格式校验第一阶段、Command menu 第二阶段、Diff / Promotion 文件 reviewed progress 第一阶段、主工作区 Skill 设置、Skill 作用域访问控制、本地 session actor、基础 accessibility 护栏、Workbench mode tablist、Inspector action 焦点交接、Skill 治理与审计面板、Skill 审计 Explorer quick filters/readable timeline/structured detail、标准 Skill bundle 导入、导入后验证清单、variant/version、candidate verification handoff、eval set version、manual eval review queue、历史查看、run matrix 多维控制、Impact 列配置与表格语义、保存历史筛选视图和对照/候选指针、run-to-run comparison、accepted verification、bundle diff、candidate promotion review、上下文命令菜单 ARIA 和快速添加 case 都能闭环。但距离成熟产品还缺少真实认证、多用户协作、自动测评策略和更深的可访问性验证。
...
| Local login 门禁 | 右侧 inspector 显示当前本地 actor，切换为 `release-manager` 等身份时必须填写本地登录码；默认 `skillhub-dev`，可用 `SKILLHUB_LOCAL_SESSION_CODE` 覆盖。面板也支持 `退出登录`，调用 `DELETE /api/session` 清除 actor cookie 并回到 `product-operator`；E2E 覆盖缺登录码的字段错误、登录后导入 skill 的 owner 来自 session actor、退出后 owner 回到默认 actor；视觉回归覆盖 session 面板。 | 完成本地 session 退出控制 |
...
| 表单验证与字段错误 | `ValidatedForm` 统一高频写入表单的 required 校验；缺字段时展示 error summary、显示需要修正的字段数量、聚焦 summary、摘要链接回字段，并通过 `WorkbenchField` 显示字段旁错误和 `aria-invalid`；后端保留 `detail` 并返回 `field_errors`，创建/更新 skill 的重复或格式错误 Skill ID 会回填到 `slug` 字段，非法 tag 会回填到 `tags` 字段，非法 `owner_ref` / role `subject_id` 会回填到 `归属` / `成员` 字段，导入 bundle 的 `SKILL.md`、frontmatter 和 zip 解析错误会回填到 `folder_files` 或 `zip_file`；批量 case parser 会返回行级错误并回填到 `batch_cases` 字段，直连批量 API 缺字段会返回 `cases[n].field`；`POST /api/eval-runs` 缺失或多余 result key 会返回 `results.<case_version_id>`；eval case 标题、Input、Expected output 和 Notes 超限会返回字段级错误；variant 名称、说明和版本说明超限会回填到 `label`、`summary` 或 `change_summary`；保存历史视图的空白、重复或超长名称会回填到 `name` 字段；accepted verification note 超过 1000 字符会回填到 `note` 字段；promotion decision note 缺失或超过 1000 字符会回填到 `decision_note` 字段；E2E 覆盖 Launchpad、QuickAddCases、错误摘要数量统计、服务端字段错误、服务端格式错误、bundle frontmatter 错误、主工作区 variant 字段错误、低频身份字段错误、批量 case 行级错误、saved view name 重名错误、accepted verification note 超长错误和 risky promotion decision note 错误，API 覆盖服务端行级字段错误、EvalRun results 精确字段错误、长度上限、saved view name 字段错误、accepted verification note 字段错误、promotion decision note 字段错误、variant 写入字段错误和 identity ref 字段错误。 | 完成 EvalRun results 精确字段校验 |
...
| Run matrix | `GET /api/skills/{skill_id}/eval-run-matrix`；History mode 展示 case x run pass/fail 矩阵；选择对照/候选后显示逐 case `修复/回退/稳定/缺失` impact；支持 impact 过滤、按 impact 分组、隐藏 run header 分数、隐藏 `Impact` 列、隐藏 `Summary` 指标列，并可把当前可见矩阵导出 CSV；E2E 覆盖多 case、多 run、impact、列配置、CSV 下载和矩阵控制。 | 完成 Summary 指标列 |
...
- TASK-068 增量验证：本地 session 退出 E2E 红灯先失败于没有 `退出登录` 按钮；绿色后目标 E2E 1 passed；local session 视觉基线更新 1 passed，全量视觉基线更新 10 passed；完整验证为 API 109 passed、Web unit 5 files / 16 tests passed、typecheck/build/audit 通过、Playwright E2E 74 passed、`git diff --check` 和任务 JSON 检查通过；完整验证记录见 `.agent/tasks/TASK-068.json`。
- TASK-069 增量验证：EvalRun results API 红灯先失败于缺失 result 返回 200；Repository 红灯先失败于缺失 result 没有抛 `FieldInvariantError`；绿色后目标 API 1 passed、目标 Repository 1 passed；完整验证为 API 111 passed、Web unit 5 files / 16 tests passed、typecheck/build/audit 通过、Playwright E2E 74 passed、`git diff --check` 和任务 JSON 检查通过；完整验证记录见 `.agent/tasks/TASK-069.json`。
- TASK-070 增量验证：Run matrix E2E 红灯先失败于没有 `Impact column`；URL state E2E 红灯先失败于没有 `Impact column`；saved view API 红灯先失败于 config 丢弃 `matrix_show_impact`；绿色后目标 API 1 passed、目标 Run matrix E2E 1 passed、目标 URL E2E 1 passed；run comparison 视觉基线更新 1 passed；完整验证为 API 111 passed、Web unit 5 files / 16 tests passed、typecheck/build/audit 通过、Playwright E2E 74 passed、`git diff --check` 和任务 JSON 检查通过；完整验证记录见 `.agent/tasks/TASK-070.json`。
...
- TASK-075 增量验证：Run matrix CSV/saved view 单元测试红灯先失败于 Summary 缺失；saved view API/Repository 红灯先失败于 `matrix_show_summary` 被丢弃；Run matrix E2E 红灯先失败于没有 `Summary column`；绿色后目标单元测试 2 files / 7 tests passed、目标 API 1 passed、目标 Repository 1 passed、目标 Run matrix/saved view E2E 2 passed、目标 URL E2E 1 passed，run comparison 视觉基线更新 1 passed；完整验证为 API 111 passed、Web unit 7 files / 23 tests passed、build/audit 通过、typecheck 初次发现测试默认值漏补后修复通过、Playwright E2E 74 passed、`git diff --check` 和任务 JSON 检查通过；完整验证记录见 `.agent/tasks/TASK-075.json`。
- TASK-076 测试组织重构：`skills-workbench.spec.ts` 从 1081 行降到 791 行，新增 293 行 `run-history-workbench.spec.ts` 承载历史记录、Run matrix、saved view、run comparison 和 case version history；定点 E2E 36 passed，完整验证为 API 111 passed、Web unit 7 files / 23 tests passed、build/typecheck/audit 通过、Playwright E2E 74 passed、`git diff --check` 和任务 JSON 检查通过；完整验证记录见 `.agent/tasks/TASK-076.json`。
- TASK-077 后端契约重构：saved view 类型校验和 config normalizer 从 `SqlSkillRepository` 抽到 `skillhub.application.saved_views`；红灯 helper 先失败于模块不存在，绿色后 helper 3 passed、Repository saved view 2 passed、API saved view 2 passed；完整验证为 API 114 passed、Web unit 7 files / 23 tests passed、build/typecheck/audit 通过、Playwright E2E 74 passed、`git diff --check` 和任务 JSON 检查通过；完整验证记录见 `.agent/tasks/TASK-077.json`。
- TASK-078 后端 read model 重构：Run matrix cases/cells 组装从 `SqlSkillRepository` 抽到 `skillhub.application.run_matrix`；红灯 builder 先失败于模块不存在，绿色后 builder 1 passed、Repository eval_run_matrix 1 passed、API eval_run_matrix 1 passed；完整验证为 API 115 passed、Web unit 7 files / 23 tests passed、build/typecheck/audit 通过、Playwright E2E 74 passed、`git diff --check` 和任务 JSON 检查通过；完整验证记录见 `.agent/tasks/TASK-078.json`。
...
5. **Run matrix 还不是完整多维表格。** 现在能保存筛选视图和对照/候选 run 指针、看 case x run pass/fail、高亮对照/候选的修复和回退，并支持 impact 过滤/分组/分数显示控制、`Impact` 列隐藏、`Summary` 指标列和当前视图 CSV 导出，但还不能做用户自定义指标公式或任意列排序。
...
2. 表单验证后续：更多嵌套字段错误回填和表格型错误定位；EvalRun results 精确校验已完成第一阶段。
3. 把 run matrix 升级为更完整的多维表格：用户自定义指标公式、任意列排序和更大的数据量策略。
