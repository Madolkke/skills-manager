...
内置函数使用专用静态签名检查参数数量、重复/未知关键字和明确不兼容类型，泛型容器不受旧种子 `array<string>` 限制。支持 `min(1, 2)`、`min([1, 2])`、`str()`、`list()`、`round(1.25, 1)` 等合法形式；未知类型保持宽容，不执行函数体。目录中的 `signatureDisplay` 用于显示实际静态签名；仅已启用并标记为内置的记录应用此规则。

条件、模板、输入绑定均使用后端静态规则。批量校验条目可携带可选 `target_schema`，响应增加可选 `assignable`；未提供时保持原契约。函数参数错误、未注册调用及绑定类型不兼容在保存和导入时由后端最终拒绝，前端保留草稿。Workflow schema version 和表达式 contractVersion 均不变。

视口小于 1180px 时工作流使用结构、编辑、预览单面板切换；节点选择与诊断定位进入编辑，校验进入预览。切换前提交编辑器待处理输入并关闭补全，桌面栏宽和折叠状态独立保留。
