...
本文档描述 SkillHub 当前 Workflow 文档格式 **schema v2**。它是 `WorkflowBundle` 的持久化结构，字段名使用 API 中的 camelCase 形式。
...
- `document_schema_version` 存在数据库 `workflows` 表中，不写入 Bundle 内部；当前值为 `2`。
- 当前只接受 schema v2，不兼容旧的 Step、Conclusion 或 Transition Key 结构。
...
`Parameter` 是输入或输出字段的声明，不保存运行时实际值。
...
| 字段 | 类型 | 必填 | 默认值 | 含义 |
| --- | --- | --- | --- | --- |
| `kind` | `string` | 是 | - | 绑定来源类型。当前领域规则支持 `workflow_input`、`step_input`、`collection_output` 和 `literal`。 |
...
| `step_input` | `{ "input_id": string }` | 引用当前步骤的输入参数。 |
...
| 字段 | 类型 | 必填 | 含义 |
| --- | --- | --- | --- |
| `parameter` | `Parameter` | 是 | 当前步骤的输入参数声明。 |
| `binding` | `Binding \| null` | 否 | 当前步骤输入的值来源。没有绑定时表示文档只声明了参数，尚未声明来源。 |

`StepInput.binding` 与 `CollectionCall.inputBindings` 方向不同：前者描述“步骤输入从哪里来”，后者描述“Collection 输入从哪里来”。
...
| `deviceRoleId` | `string \| null` | 否 | - | 执行该调用时使用的设备角色。 |
| `sampleCount` | `integer` | 否 | `1` | 需要采集或展示的样本数量。领域校验要求大于零。 |
...
| `inputs` | `StepInput[]` | 否 | `[]` | 当前步骤输入声明和来源绑定。 |
...
下面的例子表示：Collection 的 `raw_output` 输出绑定到当前步骤的 `cli_text` 输入。

```json
{
  "id": "step-analyze",
  "name": "分析命令回显",
  "description": "根据采集到的命令回显选择后续路径。",
  "isStart": false,
  "stepType": "script",
  "script": {
    "language": "python",
...
上例中的 `StepInput.binding` 是结构上的来源声明。当前后端 schema 不会据此执行脚本，也不会自动把 Collection 输出转换成 `context`；执行器如何组装运行时 context 属于后续执行器协议。
