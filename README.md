...
4. 空工作台会在主内容区显示 `SkillLaunchpad`：可以直接导入标准 Skill bundle，也可以先创建空白 skill。已有 skill 时，也可以继续用右侧 inspector 或 `Cmd/Ctrl+K` 命令菜单触发同类动作；触发后焦点会进入对应 Inspector 表单。顶部工作区模式按 tablist 建模，聚焦当前模式后可用左右方向键、Home、End 在 `概览 / 变体 / 测评 / 差异 / 历史` 间移动。
