...
class RoleMixin:
    ...
    def list_groups(self) -> list[dict[str, Any]]:
        with self.engine.connect() as connection:
            rows = connection.execute(select(tables.groups).order_by(tables.groups.c.name)).mappings().all()
    ...
    def create_group(self, *, name: str, description: str, actor: str) -> dict[str, Any]:
        ...
        with self.engine.begin() as connection:
            if connection.execute(select(tables.groups.c.id).where(tables.groups.c.name == clean_name)).scalar_one_or_none() is not None:
    ...
    def delete_group_admin(self, *, group_id: str) -> dict[str, bool]:
        deleted_at = utc_now()
        with self.engine.begin() as connection:
            group = self._group_row(connection, group_id)
