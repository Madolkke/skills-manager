...
class AssertionTemplate:
    ...
    Extension rules: keep `id` stable; describe only user-editable inputs in
    `params`; do not mutate the workdir; return `AssertionResult(False, ...)`
    for business failures; raise `InvariantError` only for invalid params or
    unsafe paths. File templates must resolve through `safe_workdir_path`.
    """
