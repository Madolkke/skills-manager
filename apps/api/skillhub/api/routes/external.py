from __future__ import annotations

from base64 import b64encode
import json

from fastapi import Depends, FastAPI, File, Form, UploadFile
from pydantic import ValidationError

from skillhub.api.database import repository_dependency
from skillhub.api.responses import parse_skill_import_payload, result_payload
from skillhub.api.schemas import ExternalSkillUpsertTagsPayload, IdentityRef, SkillVersionSemVer, VersionChangeSummary, VersionDisplayName
from skillhub.domain.errors import FieldError, FieldInvariantError
from skillhub.infrastructure.db.repositories import SqlSkillRepository


def register_external_routes(app: FastAPI) -> None:
    @app.post("/api/external/skills/upsert-zip")
    async def upsert_skill_zip(
        actor_id: IdentityRef = Form(...),
        tags: str = Form(...),
        file: UploadFile = File(...),
        display_name: VersionDisplayName | None = Form(default=None),
        version: SkillVersionSemVer | None = Form(default=None),
        change_summary: VersionChangeSummary | None = Form(default=None),
        make_current: bool = Form(default=True),
        repository: SqlSkillRepository = Depends(repository_dependency),
    ):
        archive = await file.read()
        bundle = parse_skill_import_payload({"kind": "zip", "zip_base64": b64encode(archive).decode("ascii")})
        parsed_tags = _parse_tags_form(tags)
        return result_payload(
            repository.upsert_skill_bundle_for_owner(
                owner_ref=actor_id,
                slug=bundle.slug,
                actor=actor_id,
                bundle_digest=bundle.digest,
                bundle_manifest_text=bundle.manifest_text,
                file_count=bundle.file_count,
                entry_path=bundle.entry_path,
                tags=parsed_tags,
                change_summary=change_summary,
                display_name=display_name,
                version=version,
                make_current=make_current,
            )
        )


def _parse_tags_form(value: str) -> list[dict[str, str]]:
    try:
        raw_tags = json.loads(value)
        payload = ExternalSkillUpsertTagsPayload(tags=raw_tags)
    except (json.JSONDecodeError, ValidationError) as exc:
        raise FieldInvariantError(
            "External skill upsert tags are invalid.",
            [FieldError(field="tags", message="tags 必须是有效的 Tag 数组 JSON。", code="external_skill.tags_invalid")],
        ) from exc
    return [{"group_id": tag.group_id, "value": tag.value} for tag in payload.tags]
