...
from skillhub_worker.heartbeat import record_eval_running, record_idle, renew_execution_lease
...
def run_eval_once(detail: dict[str, Any], store: SkillHubStore, client: OpencodeClient, laminar: LaminarClient, *, config) -> bool:
    ...
    execution = dict(detail.get("execution") or {})

    def heartbeat() -> bool:
        return renew_execution_lease(store, execution)
    ...
    try:
        ...
        if attempts < config.max_attempts:
            store.retry_eval_case_run_job(eval_case_run_id=eval_case_run_id, error=error, **execution)
            logger.warning("eval job retry scheduled eval_case_run_id=%s job_id=%s next_attempt=%s", eval_case_run_id, job_id, attempts + 1)
        else:
            store.fail_eval_case_run(eval_case_run_id=eval_case_run_id, error=error, **execution)
