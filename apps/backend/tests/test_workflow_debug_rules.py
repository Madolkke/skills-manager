...
@pytest.mark.parametrize("collection_type", ["log", "config"])
def test_debug_identity_rejects_ignored_collection_on_current_step(collection_type: str) -> None:
    document = executor_workflow_document()
    document["collectionSnapshots"][0]["spec"] = _unsupported_spec(collection_type)
    projection = project_workflow_document(document)

    with pytest.raises(FieldInvariantError) as error:
        build_executor_identity(projection, document=document, case=_case())

    assert error.value.field_errors[0].code == "workflow_debug.unsupported_collection_type"


def test_debug_identity_allows_ignored_collection_on_another_step() -> None:
    document = executor_workflow_document()
    ignored_definition = {
        **document["collectionSnapshots"][0],
        "id": "collection-log",
        "spec": _unsupported_spec("log"),
    }
    document["collectionSnapshots"].append(ignored_definition)
...
def _unsupported_spec(collection_type: str) -> dict:
    if collection_type == "log":
