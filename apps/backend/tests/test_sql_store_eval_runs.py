...
class SqlStoreEvalRunTest(SqlStoreTestCase):
    def test_create_eval_case_can_attach_workspace_zip_artifact(self):
        ...
        with self.engine.connect() as connection:
            ...
            case_version = self.store._case_version_detail(
                connection,
                self.store._eval_case_version_row(connection, created.eval_case_version_id),
            )
    ...
    def test_case_run_completion_is_aggregated_into_eval_run_snapshot(self):
        ...
        finished = self.store.finalize_eval_case_run(
            eval_case_run_id=queued.eval_case_run_id,
            passed=True,
            actual_output="Flagged token logging.",
            actor="tester",
        )
        run = self.store.aggregate_eval_run(
            skill_version_id=skill.skill_version_id,
            eval_set_id=case.eval_set_id,
            actor="tester",
            environment_tags=["linux"],
        ...
        latest = self.store.latest_eval_case_run_details(
            skill_version_id=skill.skill_version_id,
            eval_set_id=case.eval_set_id,
            environment_tags=["linux"],
            run_context={"os": "linux"},
    ...
    def test_eval_run_detail_returns_case_level_results_and_skill_version(self):
        ...
        second_case = self.store.create_eval_case(
            skill_id=skill.skill_id,
            title="Tenant scope",
        ...
        detail = self.store.eval_run_detail(run.eval_run_id)
    ...
    def test_eval_run_history_filters_by_skill_version_and_context(self):
        ...
        history = self.store.list_eval_runs_for_skill(
            skill_id=skill.skill_id,
            skill_version_id=candidate.skill_version_id,
            eval_set_id=case.eval_set_id,
            status="finished",
    ...
    def test_accept_eval_run_verification_is_scoped_by_run_context(self):
        ...
        linux_acceptance = self.store.accept_eval_run_verification(
            eval_run_id=linux_run.eval_run_id,
            note="Accepted on Linux.",
            actor="tester",
        )
        windows_acceptance = self.store.accept_eval_run_verification(
    ...
    def test_compare_eval_runs_returns_skill_version_context(self):
        ...
        comparison = self.store.compare_eval_runs(
