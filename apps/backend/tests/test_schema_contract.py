...
def test_alembic_chain_keeps_the_declarative_baseline() -> None:
    assert expected_revision() == "0007_workflow_agent_assistant"
    assert ScriptDirectory.from_config(alembic_config()).get_heads() == ["0007_workflow_agent_assistant"]
    ...
    assert [revision.name for revision in revisions] == [
        ...
        "0007_workflow_agent_assistant.py",
    ]
