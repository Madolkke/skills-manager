...
class WorkflowRulesTest(unittest.TestCase):
    ...
    def test_call_keys_are_globally_unique_and_multi_sample_keys_are_identifiers(self):
        document = normalize_workflow_document(self._document())
        ...
        issues = validate_workflow_document(document)
        duplicates = [item for item in issues if item["code"] == "DUPLICATE_CALL_KEY"]
        self.assertEqual({item["selection"]["itemId"] for item in duplicates}, {"opaque-call-id", "second-call"})
