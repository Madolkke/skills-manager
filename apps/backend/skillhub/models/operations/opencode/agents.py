...
class OpencodeAgentMixin:
    def list_opencode_agents_admin(self, *, include_deleted: bool = False) -> list[dict[str, Any]]:
        with self.engine.connect() as connection:
            query = select(tables.opencode_agents).order_by(tables.opencode_agents.c.name, tables.opencode_agents.c.id)
            if not include_deleted:
                query = query.where(tables.opencode_agents.c.deleted_at.is_(None))
    ...
    def list_enabled_opencode_agents(self) -> list[dict[str, Any]]:
        with self.engine.connect() as connection:
            rows = (
                connection.execute(
                    select(tables.opencode_agents)
                    .where(tables.opencode_agents.c.enabled.is_(True))
                    .where(tables.opencode_agents.c.deleted_at.is_(None))
                    .order_by(tables.opencode_agents.c.name, tables.opencode_agents.c.id)
                )
                .mappings()
                .all()
            )
    ...
    def create_opencode_agent(self, *, payload: dict[str, Any], actor: str) -> dict[str, Any]:
        ...
        with self.engine.begin() as connection:
            existing = connection.execute(select(tables.opencode_agents).where(tables.opencode_agents.c.id == agent_id)).mappings().one_or_none()
            if existing is not None and existing["deleted_at"] is None:
                raise InvariantError(f"Opencode Agent already exists: {agent_id}")
            if existing is not None:
                connection.execute(
                    update(tables.opencode_agents)
                    .where(tables.opencode_agents.c.id == agent_id)
                    .values(**row, deleted_at=None)
                )
            else:
                connection.execute(insert(tables.opencode_agents).values(**row))
    ...
    def update_opencode_agent(self, *, agent_id: str, payload: dict[str, Any], actor: str) -> dict[str, Any]:
        ...
        with self.engine.begin() as connection:
            ...
            connection.execute(update(tables.opencode_agents).where(tables.opencode_agents.c.id == clean_id).values(**values))
    ...
    def delete_opencode_agent(self, *, agent_id: str, actor: str) -> dict[str, bool]:
        ...
        with self.engine.begin() as connection:
            ...
            connection.execute(
                update(tables.opencode_agents)
                .where(tables.opencode_agents.c.id == clean_id)
    ...
    def _opencode_agent_row(self, connection, agent_id: str, *, include_deleted: bool):
        query = select(tables.opencode_agents).where(tables.opencode_agents.c.id == agent_id)
        if not include_deleted:
            query = query.where(tables.opencode_agents.c.deleted_at.is_(None))
