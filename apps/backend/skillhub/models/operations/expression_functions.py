...
class ExpressionFunctionStoreMixin:
    ...
    def expression_function_contract(self) -> dict[str, dict[str, Any]]:
        ...
        for item in self.list_expression_functions(include_disabled=False):
            signature = {
                "name": item["name"], "parameters": _parameter_names(item["parameterSchema"]),
                "parameterSchema": item["parameterSchema"], "returns": item["returnSchema"].get("type", "any"),
                "returnSchema": item["returnSchema"], "description": item["description"],
                "enabled": item["enabled"], "isBuiltin": item["isBuiltin"], "language": item["language"],
            }
...
def _clean_expression_function_payload(payload: Mapping[str, Any], *, actor: str, function_id: str | None) -> dict[str, Any]:
    ...
    parameter_schema["x-parameter-order"] = list(parameter_schema["properties"])
    now = utc_now()
    return {
...
def _validate_schema(value: Any, *, root_object: bool, label: str, identifier_properties: bool = False) -> dict[str, Any]:
    ...
    if schema_type == "object":
        ...
        for key, child in properties.items():
            ...
            normalized_properties[property_name] = _validate_schema(child, root_object=False, label=f"{label}.{property_name}", identifier_properties=False)
        if "additionalProperties" in result and not isinstance(result["additionalProperties"], bool):
            raise InvariantError(f"{label}.additionalProperties must be boolean.")
        order = result.get("x-parameter-order")
        if order is not None:
            if not isinstance(order, list) or not all(isinstance(key, str) for key in order) or len(order) != len(set(order)) or set(order) != set(normalized_properties):
                raise InvariantError(f"{label}.x-parameter-order must list each property exactly once.")
            normalized_properties = {key: normalized_properties[key] for key in order}
        result["properties"] = normalized_properties
        result["required"] = list(required)
        result.setdefault("additionalProperties", False)
    elif schema_type == "array":
        result["items"] = _validate_schema(result.get("items"), root_object=False, label=f"{label}.items", identifier_properties=False)
    return result
...
def _expression_function_row(item: Any) -> dict[str, Any]:
    return {
        ...
        "parameterSchema": {**item.parameter_schema, "properties": {key: item.parameter_schema["properties"][key] for key in _parameter_names(item.parameter_schema)}},
...
def _parameter_names(schema: Mapping[str, Any]) -> list[str]:
    return list(schema.get("x-parameter-order", schema.get("properties", {}).keys()))
