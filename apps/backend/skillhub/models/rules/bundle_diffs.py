...
def build_bundle_file_diff(
    left_files: list[dict[str, Any]],
    right_files: list[dict[str, Any]],
) -> dict[str, Any]:
    """Build a deterministic text-aware diff for two Skill Bundle file lists."""
    ...
    for path in sorted(set(left_by_path) | set(right_by_path)):
        ...
        status = _file_status(left_file, right_file)
        summary[status] += 1
        if status == "unchanged":
            continue
        ...
        if not binary and _is_optional_text(left_text) and _is_optional_text(right_text):
            diff_file["hunks"] = line_diff_hunks(left_text, right_text)
        files.append(diff_file)
    return {"summary": summary, "files": files}


def line_diff_hunks(left_text: str | None, right_text: str | None) -> list[dict[str, Any]]:
    """Return a full-file line diff in the Bundle API hunk shape."""
...
def _file_status(left_file: dict[str, Any] | None, right_file: dict[str, Any] | None) -> str:
