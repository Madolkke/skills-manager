...
这条 Playwright smoke 固化 SkillHub Web 的核心闭环，作为正式版每轮推送前的真实操作验证。
...
- Web: `http://127.0.0.1:13110/skills`

测试使用临时 SQLite 数据库，不写入 `.data/skillhub.sqlite3`。失败时 Playwright 会把 trace 和错误上下文写入 `apps/web/test-results/`；该目录已加入 `.gitignore`，不应提交。
...
它刻意只保留一条稳定 happy path，用来证明核心业务闭环可跑。更细的 helper 逻辑继续放在 Vitest；视觉布局回归由 [Web 视觉 Smoke](visual-smoke.md) 覆盖。
