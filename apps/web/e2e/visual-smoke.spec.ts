...
test("captures the five product reference pages", async ({ page, request }) => {
  ...
  await expect(page.locator(".versions-workspace")).toBeVisible();
  await expect(page.locator(".version-summary-panel")).toContainText("当前版本");
  await expect(page.locator(".version-summary-panel")).toContainText(/v\d+/);
  await expect(page.locator(".version-node-strip .version-node.active")).toContainText("当前");
  await expect(page.locator(".version-files-panel")).toContainText("Bundle 内容");
