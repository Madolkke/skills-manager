...
test("operator can complete the SkillHub core flow", async ({ page }, testInfo) => {
  const app = new SkillHubE2E(page);
  const bundle = new SkillBundleFixture(testInfo.outputPath("skill-bundle"));
  const skillSlug = `e2e-core-reviewer-${Date.now()}`;
  const caseTitle = "E2E: core flow has clear primary action";
