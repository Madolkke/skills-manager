...
import {
  ...
  workflowDebugDraftDirty, workflowDebugDraftValid, workflowDebugStepHasUnsupportedCollections,
  type WorkflowDebugCaseDraft,
} from "../form";
...
const unsupportedCollections = computed(() => workflowDebugStepHasUnsupportedCollections(props.bundle, props.step));
const startDisabledReason = computed(() => {
  if (unsupportedCollections.value) return "当前 Step 包含日志或配置采集，暂不支持单步调试。";
...
<template>
  <Modal :open="props.open" :title="`单步调试 · ${props.step.name}`" :description="`基于已保存的 Workflow revision ${props.revision} 管理调试例和验证直接跳转。`" size="workspace" motion="workflow" @close="emit('close')">
    <div class="workflow-debug-shell">
      ...
      <div v-else-if="unsupportedCollections" class="workflow-debug-dirty-banner">当前 Step 包含日志或配置采集。你仍可管理调试例和查看历史，但暂时不能发起运行。</div>
