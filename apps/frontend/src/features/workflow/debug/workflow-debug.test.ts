...
describe("Workflow single-step debug", () => {
  ...
  it("keeps case management available while an unsupported collection blocks run start", async () => {
    const bundle = debugBundle();
    bundle.collectionSnapshots[0]!.spec = {
      collectionType: "log", sqlDialect: "duckdb", queries: [], outputSamples: [],
    };
    const step = bundle.workflow.nodes[0] as WorkflowStep;
    const client = fakeClient({ listCases: vi.fn().mockResolvedValue([debugCase()]) });
    const wrapper = mount(WorkflowDebugModal, {
      attachTo: document.body,
      props: { open: true, skillId: "skill-1", bundle, revision: 4, step, workflowDirty: false, client },
    });
    await flushPromises();

    expect(document.body.textContent).toContain("当前 Step 包含日志或配置采集");
    const start = [...document.body.querySelectorAll<HTMLButtonElement>("button")]
      .find((button) => button.textContent?.includes("开始调试"))!;
    expect(start.dataset.disabled).toBe("true");
    expect(document.body.querySelector<HTMLButtonElement>('button[aria-label="新建调试例"]')?.disabled).toBe(false);
    expect(document.body.textContent).toContain("提供采集结果");
    start.click();
    expect(client.startRun).not.toHaveBeenCalled();
    wrapper.unmount();
  });
