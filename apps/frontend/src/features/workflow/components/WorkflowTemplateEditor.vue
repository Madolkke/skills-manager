...
onMounted(() => {
  ...
  view = new EditorView({ parent: host.value, state: EditorState.create({ doc: props.value, extensions: [autocompletion({ override: [completionSource], activateOnTyping: false, interactionDelay: 0, defaultKeymap: false }), keymap.of([{ key: "Mod-Space", run: startCompletion }, ...completionKeymap]), EditorView.lineWrapping, readonlyCompartment.of(readonlyExtensions(props.readonly)), editorPlaceholder(props.placeholder), EditorView.contentAttributes.of({ "aria-label": props.ariaLabel, "aria-autocomplete": "list" }), EditorView.domEventHandlers({ keydown(event, currentView) { if (event.key === "Tab" || event.key === "Enter") return acceptWorkflowExpressionCompletion(currentView); return false; }, blur(_event, currentView) { clearAutomaticCompletionTimer(); flushPendingChange(); closeCompletion(currentView); return false; } }), EditorView.updateListener.of((update: ViewUpdate) => { if (!update.docChanged || external) return; queueChange(update.state.doc.toString()); scheduleAutomaticCompletion(update); })] }) });
