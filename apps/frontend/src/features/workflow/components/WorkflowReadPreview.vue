...
const roleName = (id?: string) => props.bundle.workflow.deviceRoles.find((item) => item.id === id)?.name ?? "未指定角色";
...
<template>
  <div class="workflow-read-preview">
    ...
    <section class="workflow-read-steps"><h2>排查步骤</h2><article v-for="(step, index) in workflowSteps(props.bundle)" :key="step.id"><button type="button" @click="emit('select', { type: 'step', id: step.id })"><b>{{ String(index + 1).padStart(2, "0") }}</b><span><strong>{{ step.name }}</strong><small>{{ step.description || "尚未填写步骤说明" }}</small></span><i v-if="step.isStart"><Play :size="10" />起点</i></button><div class="workflow-read-step-body"><div v-for="call in step.collectionCalls" :key="call.id" class="workflow-read-call"><TerminalSquare :size="14" /><div><strong>{{ call.name }}</strong><small>{{ roleName(call.deviceRoleId) }} · {{ call.sampleCount }} 份样本</small><code>{{ findCollection(props.catalog, call.definition)?.spec.commandTemplate || "采集定义不可用" }}</code></div></div><div v-for="path in step.topology" :key="path.id" class="workflow-read-path"><span>{{ path.conditionText || "无条件" }}</span><ArrowRight :size="14" /><strong>{{ targetName(path.target.id) }}</strong></div></div></article></section>
