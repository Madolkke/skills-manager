...
<template>
  <section class="workflow-document workflow-metadata-document">
    ...
    <div class="workflow-form-grid">
      <label class="field-label span-2"><span>工作流名称</span><input data-workflow-field="name" :value="props.metadata.name" :disabled="props.readonly" @input="emit('change', { name: ($event.target as HTMLInputElement).value })" /></label>
      ...
      <label class="field-label span-2"><span>工作流说明</span><textarea data-workflow-field="description" rows="7" :value="props.metadata.description" :disabled="props.readonly" @input="emit('change', { description: ($event.target as HTMLTextAreaElement).value })" /></label>
