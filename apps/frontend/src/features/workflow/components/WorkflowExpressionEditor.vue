...
<template>
  <div class="workflow-expression-field"><div ref="host" :class="['workflow-expression-editor', props.readonly && 'is-readonly', props.diagnostics.length && 'has-warning']" /><ul v-if="props.diagnostics.length" class="workflow-expression-diagnostics"><li v-for="item in props.diagnostics" :key="`${item.code}:${item.start}`">{{ item.message }} <small>{{ item.start + 1 }}–{{ item.end }}</small></li></ul></div>
</template>
