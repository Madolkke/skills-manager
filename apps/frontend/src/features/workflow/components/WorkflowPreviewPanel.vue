...
<template>
  <section v-bind="$attrs" class="workflow-preview-panel">
    <div :class="['workflow-preview-tabs', `is-${tab}`]" role="tablist" aria-label="Workflow 预览">
      <button v-for="item in [{ id: 'graph', label: '流程图' }, { id: 'read', label: '阅读视图' }, { id: 'validation', label: `校验 ${props.issues.length}` }]" :key="item.id" :class="tab === item.id && 'active'" type="button" role="tab" :aria-selected="tab === item.id" @click="tab = item.id as typeof tab">{{ item.label }}</button>
    </div>
    <Transition name="workflow-preview-switch" mode="out-in">
      <WorkflowGraph v-if="tab === 'graph'" key="graph" :bundle="props.bundle" :issues="props.issues" :selected="props.selection" :direction="direction" compact allow-fullscreen @select="emit('select', $event)" @update:direction="direction = $event" @fullscreen="graphFullscreen = true" />
  ...
  <WorkflowGraphModal :open="graphFullscreen" :bundle="props.bundle" :issues="props.issues" :selection="props.selection ?? { type: 'metadata' }" :direction="direction" @close="graphFullscreen = false" @select="graphFullscreen = false; emit('select', $event)" @update:direction="direction = $event" />
</template>
