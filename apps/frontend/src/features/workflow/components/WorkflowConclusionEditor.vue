...
<template>
  <section class="workflow-document">
    <header class="workflow-document-head"><span><Flag :size="18" /></span><div><small>CONCLUSION</small><h2>{{ props.conclusion.name }}</h2><p>结论是流程终点，描述根因和修复建议。</p></div><UiIconButton label="删除结论" variant="danger" :disabled="props.readonly" @click="emit('remove')"><Trash2 /></UiIconButton></header>
