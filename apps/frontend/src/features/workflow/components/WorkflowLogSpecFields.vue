...
import type { CollectionDefinition, LogCollectionSpec, WorkflowLogSchemaCatalog, WorkflowValidationIssue } from "../../../types";
...
function queryIssue(queryId: string, suffix: string): boolean {
  return Boolean(props.issues?.some((item) => item.selection.type === "collection" && item.selection.itemId === queryId && item.selection.field?.endsWith(suffix)));
}
...
<template>
  <section class="workflow-field-section workflow-log-spec">
    ...
    <article v-for="query in (props.definition.spec.collectionType === 'log' ? props.definition.spec.queries : [])" :key="query.id" class="workflow-log-query">
      ...
      <label :class="['field-label', queryIssue(query.id, '.outputIds') && 'field-invalid']"><span>输出字段</span><select multiple :value="query.outputIds" :disabled="props.readonly" :aria-invalid="queryIssue(query.id, '.outputIds')" @change="updateOutputIds(query.id, $event)"><option v-for="output in props.definition.outputs" :key="output.id" :value="output.id">{{ output.key || output.schema.title || output.id }}</option></select><small>每个输出只能归属一条查询；SQL alias 必须等于变量名。</small></label>
      <label :class="['field-label', queryIssue(query.id, '.sql') && 'field-invalid']"><span>SQL</span><textarea class="workflow-log-sql-input" rows="8" spellcheck="false" placeholder="SELECT count(*) AS error_count FROM logs" :value="query.sql" :disabled="props.readonly" :aria-invalid="queryIssue(query.id, '.sql')" @input="update((spec) => { const target = spec.queries.find((item) => item.id === query.id); if (target) target.sql = ($event.target as HTMLTextAreaElement).value; })" /></label>
    </article>
