...
<template>
  <Transition name="workflow-editor-switch" mode="out-in">
    <div v-if="props.editor.bundle.value" :key="contentKey" class="workflow-editor-content">
      ...
      <WorkflowSettingsEditor v-else-if="props.editor.selection.value.type === 'inputs' || props.editor.selection.value.type === 'roles'" :inputs="props.editor.bundle.value.workflow.inputs" :roles="props.editor.bundle.value.workflow.deviceRoles" :issues="props.editor.issues.value" :target="props.editor.selection.value.type" :readonly="props.readonly" @add-input="props.editor.addInput" @update-input="props.editor.updateInput" @remove-input="props.editor.removeInput" @add-role="props.editor.addDeviceRole" @update-role="props.editor.updateDeviceRole" @remove-role="props.editor.removeDeviceRole" />
      ...
      <WorkflowConclusionEditor v-else-if="selectedConclusion" :conclusion="selectedConclusion" :bundle="props.editor.bundle.value" :issues="props.editor.issues.value" :readonly="props.readonly" @change="props.editor.updateConclusion(selectedConclusion.id, $event)" @remove="emit('request-delete', 'conclusion', selectedConclusion.id)" @predecessor-open="(id) => select({ type: 'step', id })" />
    </div>
  </Transition>
