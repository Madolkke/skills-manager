    ...
    <section ref="inputSection" :class="['workflow-settings-section', props.target === 'inputs' && 'is-target']" tabindex="-1" aria-labelledby="workflow-inputs-heading">
      ...
      <div class="workflow-settings-list">
        <div v-for="item in props.inputs" :key="item.id" class="workflow-setting-row is-input">
          <input class="workflow-key-input" :value="item.key" aria-label="输入 Key" :disabled="props.readonly" @input="emit('update-input', item.id, { key: ($event.target as HTMLInputElement).value })" />
          <input :value="item.name" aria-label="输入名称" :disabled="props.readonly" @input="emit('update-input', item.id, { name: ($event.target as HTMLInputElement).value })" />
          <input :value="item.description" aria-label="输入说明" :disabled="props.readonly" @input="emit('update-input', item.id, { description: ($event.target as HTMLInputElement).value })" />
          <select :value="item.dataType" aria-label="输入类型" :disabled="props.readonly" @change="emit('update-input', item.id, { dataType: ($event.target as HTMLSelectElement).value })">
            <option v-for="type in ['string', 'integer', 'number', 'boolean', 'array', 'object']" :key="type" :value="type">{{ type }}</option>
          </select>
          <label class="workflow-check"><input type="checkbox" :checked="item.required" :disabled="props.readonly" @change="emit('update-input', item.id, { required: ($event.target as HTMLInputElement).checked })" />必填</label>
    ...
    <section ref="roleSection" :class="['workflow-settings-section', props.target === 'roles' && 'is-target']" tabindex="-1" aria-labelledby="workflow-roles-heading">
      ...
      <div class="workflow-settings-list">
        <div v-for="item in props.roles" :key="item.id" class="workflow-setting-row is-role">
          <input class="workflow-key-input" :value="item.key" aria-label="角色 Key" :disabled="props.readonly" @input="emit('update-role', item.id, { key: ($event.target as HTMLInputElement).value })" />
          <input :value="item.name" aria-label="角色名称" :disabled="props.readonly" @input="emit('update-role', item.id, { name: ($event.target as HTMLInputElement).value })" />
          <input :value="item.description" aria-label="角色说明" :disabled="props.readonly" @input="emit('update-role', item.id, { description: ($event.target as HTMLInputElement).value })" />
          <label class="workflow-check"><input type="checkbox" :checked="item.required" :disabled="props.readonly" @change="emit('update-role', item.id, { required: ($event.target as HTMLInputElement).checked })" />必填</label>
