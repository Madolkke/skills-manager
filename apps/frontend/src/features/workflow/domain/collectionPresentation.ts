...
export function isFunctionCollection(definition?: CollectionDefinition): boolean {
...
export function collectionTypeLabel(definition?: CollectionDefinition): string {
  ...
  if (isFunctionCollection(definition)) return "自定义函数";
  return "CLI 命令";
}
...
export function collectionContentSummary(definition?: CollectionDefinition): string {
  ...
  if (definition.spec.collectionType === "function") return definition.spec.source.split(/\r?\n/, 1)[0]?.trim() || "未填写 Python 代码";
...
export function collectionSearchText(definition: CollectionDefinition): string {
  const content = definition.spec.collectionType === "cli"
    ...
    : definition.spec.collectionType === "function"
