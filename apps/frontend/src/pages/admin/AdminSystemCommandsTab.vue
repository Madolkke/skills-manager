...
function loadDraft(value?: SystemCommand): void {
  ...
  jsonSchema.value = JSON.stringify(schemaModel.value, null, 2);
...
function addSample(): void { samples.value.push(newSample()); }
...
function onSchemaJsonInput(value: string): void {
  jsonSchema.value = value;
  try {
    const parsed = JSON.parse(value);
    const errors = validateSchema(parsed);
    if (errors.length) { schemaJsonError.value = errors[0]!; return; }
    schemaModel.value = normalizeSchema(parsed);
    schemaJsonError.value = "";
  } catch {
    schemaJsonError.value = "输出 Schema JSON 格式不正确。";
  }
}

function formatSchemaJson(): void {
  try { onSchemaJsonInput(JSON.stringify(JSON.parse(jsonSchema.value), null, 2)); } catch { schemaJsonError.value = "输出 Schema JSON 无法格式化。"; }
}
...
function validateDraft(): string[] {
  ...
  if (schemaJsonError.value) errors.push(schemaJsonError.value);
