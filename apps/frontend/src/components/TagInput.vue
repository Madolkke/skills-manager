...
const props = withDefaults(defineProps<{ value: string[]; suggestions?: string[]; placeholder?: string; disabled?: boolean }>(), {
  suggestions: () => [],
...
<template>
  <div class="tag-input">
    <div class="tag-box">
      <span v-for="tag in value" :key="tag" class="tag-chip editable" :title="tag">
        <span class="tag-chip-label">{{ tag }}</span>
        <button v-if="!disabled" type="button" :aria-label="`移除 ${tag}`" @click="emit('change', value.filter((item) => item !== tag))">
          <X :size="13" />
        </button>
      </span>
      <input
    ...
    <div v-if="!disabled && available.length > 0" class="tag-suggestions">
      <button
        ...
        @click="addTag(tag)"
      >
        <span class="tag-chip-label" :title="tag">{{ tag }}</span>
      </button>
    </div>
  </div>
</template>
