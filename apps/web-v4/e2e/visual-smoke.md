...
测试会启动独立的临时 SQLite 数据库，用固定 seed 创建 Skill、变体、case、EvalRun，并用 `1586x992` viewport 截图。该尺寸与参考图尺寸一致或接近。Hub 首页会断言最近测评默认显示 6 条，并且 `查看全部` / `收起` 能在同一真实列表内切换 6/7 条记录；Skill 概览页会断言 summary 身份卡显示真实根目录和维护者，也会断言 bundle 文件树存在根目录、子目录和叶子文件节点，避免退回轻量标题区或平铺分组列表；测评集页会断言 case 行包含序号、case version、当前状态和生命周期状态；手动测评页会断言进度区包含聚合摘要、总 case 数和结果分布条，case 行包含紧凑序号、状态点和数字快捷键，并会点击两个 `查看详情` 验证 VariantVersion / EvalSetVersion 真实绑定信息；变体页会断言 inspector 使用水平版本线、当前版本摘要、`Bundle diff` 和 `查看该版本详情`，避免退回纵向版本卡片或无真实行为的底部按钮。
